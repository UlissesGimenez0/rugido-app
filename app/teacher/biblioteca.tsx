...
export default function BibliotecaTreinos() {
    ...
    // Estados do Modal de Criação (agora com type_id)
    ...
    const loadData = async () => {
        try {
            setLoading(true);
            const [tData, typesData] = await Promise.all([
                getTemplatesTreino(),
                getWorkoutTypes()
            ]);
            setTemplates(tData);
    ...
    const handleCreateTemplate = async () => {
        ...
        try {
            setCreating(true);
            await createTemplateTreino(form.name, form.description, user.id, form.type_id);
            setCreateVisible(false);
            setForm({ name: "", description: "", type_id: "" });
            loadData();
        } catch (error: any) { alert("Erro ao criar molde: " + error.message); }
    ...
    const handleClone = async (studentId: string, studentName: string) => {
        ...
        try {
            ...
            Alert.alert("Sucesso!", `O treino '${selectedTemplate.name}' foi enviado para ${studentName} com sucesso!`);
                ...
                renderItem={({ item }) => (
                    <Card style={styles.card}>
                        ...
                        <View style={styles.actionRow}>
                            <TouchableOpacity style={[styles.actionBtn, { backgroundColor: "rgba(255,255,255,0.08)" }]} onPress={() => router.push(`/teacher/treino/${item.id}/exercicios`)}>
            ...
            <Modal visible={createVisible} transparent animationType="slide">
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <View style={{ minHeight: 60, marginBottom: 15 }}>
                        ...
                        <TextInput style={styles.input} placeholder="Nome (Ex: Ficha A)" placeholderTextColor="#888" value={form.name} onChangeText={(t) => setForm({ ...form, name: t })} />
                        ...
                        <View style={styles.modalButtons}>
                            ...
                            <TouchableOpacity style={[styles.confirmBtn, creating && { opacity: 0.7 }]} onPress={handleCreateTemplate} disabled={creating}>
                                {creating ? <ActivityIndicator color="#111" /> : <Text style={styles.confirmBtnText}>Criar</Text>}
                            </TouchableOpacity>
                        </View>
                    </View>
                </View>
            ...
            <Modal visible={cloneVisible} transparent animationType="fade">
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <Text style={styles.modalSubtitle}>Selecione o aluno que vai receber o treino: <Text style={{ color: "#fff", fontWeight: "bold" }}>{selectedTemplate?.name}</Text></Text>
                        {cloning ? (
                            <View style={{ padding: 40, alignItems: "center" }}>
                                <ActivityIndicator color="#FF005E" size="large" />
                                <Text style={{ color: "#fff", marginTop: 15 }}>A enviar treino...</Text>
                            </View>
                        ) : (
                            <FlatList
                                data={alunos}
                                ...
                                renderItem={({ item }) => (
                                    <TouchableOpacity style={styles.studentChip} onPress={() => handleClone(item.id, item.name)}>
                                        <View style={styles.avatarMini}><Text style={styles.avatarMiniText}>{item.name.charAt(0)}</Text></View>
...
const styles = StyleSheet.create({
    ...
    inputLabel: { color: "#aaa", marginBottom: 8, fontSize: 14, fontWeight: "600", width: "100%", textAlign: "left" },
